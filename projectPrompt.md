...
- `config.max_open_sockets = 10`, paired with `CONFIG_LWIP_MAX_SOCKETS=22` in `sdkconfig.defaults` (the ESP-IDF default of 10 is the whole system's socket budget shared with mDNS/WiFi, not just the HTTP server's, and can be exhausted by a single page load — index.html, style.css, app.js, favicon.ico, plus the `/ws` upgrade — causing `accept()` to fail system-wide with errno 23/ENFILE and the server becoming completely unreachable). `httpd_start()` itself also reserves 3 sockets from `CONFIG_LWIP_MAX_SOCKETS` on top of `max_open_sockets`, so with the old value of 16 the webserver alone already claimed 13 of 16, leaving only 3 for the rest of the system — this was confirmed on hardware (`httpd_accept_conn: error in accept (23)` after a few minutes with zero completed HTTP/WS requests logged) and is why the value was raised to 22.
...
- No `config.ws_post_handshake_cb` on the `/ws` `httpd_uri_t`. ESP-IDF v6.0.2's `esp_http_server` source (`httpd_uri.c`) confirms that for `is_websocket=true` URIs the framework sends the `101 Switching Protocols` response itself and never calls the registered `.handler` for that initial GET, but sending a frame from `.ws_post_handshake_cb` was found to break the normal recv/response flow on this IDF version (documented in `/useWebSockets.md` section 5, from a prior project on the same IDF version). Instead, `check_takeover()` in `webserver_api.c` runs on every inbound `/ws` data frame (from `ws_handler()`), enforces the single-active-client rule, and sends `"hello"` there; `components/webserver/gui/app.js`'s `connectWebSocket()` calls `refreshFileList()` from `ws.onopen` so the client always speaks first, since the server cannot push anything before it sees a data frame from a new connection.
- `config.keep_alive_enable = true` with short `keep_alive_idle/interval/count` (5s/3s/3 retries). Without `SO_KEEPALIVE`, lwIP only detects a WebSocket peer that vanished without a clean close (WiFi drop, laptop sleep, browser tab killed) once it gives up retransmitting unacked data after `CONFIG_LWIP_TCP_MAXRTX` attempts — confirmed on hardware as a ~300 second stall (`httpd_ws_get_frame_type: ... socket FD invalid, closing socket now`) during which the socket sits "open" but dead and the GUI is unusable until it's reclaimed.
- `esp_wifi_set_ps(WIFI_PS_NONE)` called from `on_wifi_connected()` in `webserver.c`. ESP-IDF's default WiFi power-save mode (`WIFI_PS_MIN_MODEM`, visible in the boot log as `wifi:pm start, type: 1`) lets the radio sleep between beacons; confirmed on hardware to cause the browser's `/ws` connection to go silently unresponsive for several seconds at a time and fire `onclose` (showing "Connection lost or taken over") even though the ESP32-side session is still perfectly healthy — the server only notices anything wrong later, when the browser's reconnect attempt triggers the single-client takeover logic.

Do not lower these limits or revert to the ESP-IDF httpd defaults without re-validating on hardware; the symptoms of a regression are silent — the server simply stops accepting connections after some period of use, with `httpd_accept_conn: error in accept (23)` in the log, or a dead WebSocket session lingers for minutes before being reclaimed.
